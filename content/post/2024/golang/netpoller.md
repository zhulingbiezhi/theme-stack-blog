...
date: 2024-03-20
image: http://img.ququ123.top/img/netpoller-goroutines-scheduling.png
categories: 
    - golang
    - pipe
...
![](http://img.ququ123.top/img/slide_13.jpg)
...
![](http://img.ququ123.top/img/5-io-model.jpg)
...
![](http://img.ququ123.top/img/non-blocking-io.png)
...
理解 select 的关键在于理解 fd\_set，为说明方便，取 fd\_set 长度为 1 字节，fd\_set 中的每一 bit 可以对应一个文件描述符 fd，则 1 字节长的 fd\_set 最大可以对应 8 个 fd。select 的调用过程如下：
...
![](http://img.ququ123.top/img/epoll-principle.png)
...
![](http://img.ququ123.top/img/go-network-server.png)
...
![](http://img.ququ123.top/img/netpoller-goroutines-scheduling.png)
...
![](http://img.ququ123.top/img/goroutine-scheduler-model.png)
...
通常设置一个主线程负责做 event-loop 事件循环和 I/O 读写，通过 select/poll/epoll\_wait 等系统调用监听 I/O 事件，业务逻辑提交给其他工作线程去做。而所谓『非阻塞 I/O』的核心思想是指避免阻塞在 read() 或者 write() 或者其他的 I/O 系统调用上，这样可以最大限度的复用 event-loop 线程，让一个线程能服务于多个 sockets。在 Reactor 模式中，I/O 线程只能阻塞在 I/O multiplexing 函数上（select/poll/epoll\_wait）。
...
accept 连接以及 conn 上的读写操作若是在主线程完成，则要求是非阻塞 I/O，因为 Reactor 模式一条最重要的原则就是：I/O 操作不能阻塞 event-loop 事件循环。**实际上 event loop 可能也可以是多线程的，只是一个线程里只有一个 select/poll/epoll\_wait**。
...
![](http://img.ququ123.top/img/conn-rate.png)

那么为每一个连接指派一个 goroutine 就显得太过奢侈了，而 Reactor 模式这种利用 I/O 多路复用进而只需要使用少量线程即可管理海量连接的设计就可以在这样网络业务中大显身手了：

![MultiReactors.png](http://img.ququ123.top/img/multi-reactors.png)
...
![multireactorsthreadpool.png](http://img.ququ123.top/img/multi-reactors%2Bthread-pool.png)
...
+   [关于select函数中timeval和fd\_set重新设置的问题](https://blog.csdn.net/starflame/article/details/7860091)
