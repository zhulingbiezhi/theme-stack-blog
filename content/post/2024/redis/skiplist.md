...
date: 2024-03-19
image: http://img.ququ123.top/img/skiplist.png
categories: 
    - redis
    - 数据结构
    - 原理
...
[![有序链表结构图](http://img.ququ123.top/img/sorted_linked_list.png)](sorted_linked_list.png)

在这样一个链表中，如果我们要查找某个数据，那么需要从头开始逐个进行比较，直到找到包含数据的那个节点，或者找到第一个比给定数据大的节点为止（没找到）。也就是说，时间复杂度为O(n)。同样，当我们要插入新数据的时候，也要经历同样的查找过程，从而确定插入位置。

假如我们每相邻两个节点增加一个指针，让指针指向下下个节点，如下图：

[![每两个节点增加一个跳跃指针的有序链表](http://img.ququ123.top/img/skip2node_linked_list.png)](skip2node_linked_list.png)

这样所有新增加的指针连成了一个新的链表，但它包含的节点个数只有原来的一半（上图中是7, 19, 26）。现在当我们想查找数据的时候，可以先沿着这个新链表进行查找。当碰到比待查数据大的节点时，再回到原来的链表中进行查找。比如，我们想查找23，查找的路径是沿着下图中标红的指针所指向的方向进行的：

[![一个搜索路径的例子](http://img.ququ123.top/img/search_path_on_skip2node_list.png)](search_path_on_skip2node_list.png)
...
[![两层跳跃指针](http://img.ququ123.top/img/skip2node_level3_linked_list.png)](skip2node_level3_linked_list.png)
...
[![skiplist插入形成过程](http://img.ququ123.top/img/skiplist_insertions.png)](skiplist_insertions.png)
...
[![skiplist上的查找路径展示](http://img.ququ123.top/img/search_path_on_skiplist.png)](search_path_on_skiplist.png)
...
[![skiplist平均层数计算](http://img.ququ123.top/img/skiplist_avg_level.png)](skiplist_avg_level.png)
...
[![skiplist沿查找路径回溯](http://img.ququ123.top/img/skiplist_backwards.png)](skiplist_backwards.png)
...
[![Redis skiplist结构举例](http://img.ququ123.top/img/redis_skiplist_example.png)](redis_skiplist_example.png)
